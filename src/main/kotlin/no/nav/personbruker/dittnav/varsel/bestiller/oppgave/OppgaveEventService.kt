...
class OppgaveEventService(
        private val kafkaProducer: KafkaProducer<Oppgave>,
    ...
    override suspend fun processEvents(events: ConsumerRecords<Nokkel, Oppgave>) {
        ...
        metricsProbe.runWithMetrics(eventType = OPPGAVE) {
            events.forEach { event ->
                try {
                    val oppgaveEksternVarslingKey = createKeyForEvent(event.getNonNullKey())
                    val oppgaveEksternVarslingEvent = createOppgaveEksternVarslingForEvent(event.value())

                    successfullyValidatedEvents.add(RecordKeyValueWrapper(oppgaveEksternVarslingKey, oppgaveEksternVarslingEvent))
